...
ALLOWED_TABLES = ["sales_figures_since_2025"]


def test_valid_select_passes() -> None:
    validate_sql("SELECT * FROM sales_figures_since_2025 LIMIT 10", ALLOWED_TABLES)
...
def test_valid_with_select_passes() -> None:
    validate_sql(
        """
        WITH recent AS (
            SELECT * FROM sales_figures_since_2025 LIMIT 10
        )
...
def test_drop_table_fails() -> None:
    with pytest.raises(SQLValidationError, match="DROP statements are not allowed"):
        validate_sql("DROP TABLE sales_figures_since_2025", ALLOWED_TABLES)


def test_delete_fails() -> None:
    with pytest.raises(SQLValidationError, match="DELETE statements are not allowed"):
        validate_sql("DELETE FROM sales_figures_since_2025", ALLOWED_TABLES)
...
def test_read_parquet_function_fails() -> None:
    with pytest.raises(SQLValidationError, match="read_parquet"):
        validate_sql(
            "SELECT * FROM read_parquet('data/sales/sales_figures_since_2025.parquet')",
            ALLOWED_TABLES,
        )


def test_query_without_limit_is_wrapped_with_max_limit() -> None:
    wrapped_query = limit_query("SELECT * FROM sales_figures_since_2025")

    assert wrapped_query == (
        "SELECT * FROM (SELECT * FROM sales_figures_since_2025) "
        f"AS validated_query LIMIT {MAX_RETURNED_ROWS}"
    )
