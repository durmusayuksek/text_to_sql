...
def test_query_agent_returns_catalog_sql() -> None:
    result = generate_sql("Show sample rows")

    assert "FROM qa" in result.sql
    assert result.explanation
    assert result.confidence == "high"
    assert result.assumptions == []


def test_query_agent_can_generate_sql_using_allowed_catalog_tables() -> None:
    result = generate_sql("Show passenger volume by route")

    assert "FROM pax_forecast" in result.sql
    assert "forecast_pax" in result.sql
    assert result.confidence == "high"
...
def test_query_agent_populates_schema_context() -> None:
    ...
    assert "Data catalog for Question & Answer Text-to-SQL" in result.schema_context_used
...
def test_api_ask_accepts_question_without_module_id(monkeypatch) -> None:
    ...
    def fake_generate_sql(question: str) -> QueryAgentResult:
    ...
    monkeypatch.setattr("app.ask_service.generate_sql", fake_generate_sql)
    monkeypatch.setattr(
...
def test_api_ask_blocks_destructive_intent_before_query_agent(monkeypatch) -> None:
    def fail_if_called(question: str) -> QueryAgentResult:
        raise AssertionError("Query Agent should not be called for destructive intent.")

    monkeypatch.setattr("app.ask_service.generate_sql", fail_if_called)
...
def test_openai_mode_uses_mocked_openai_response(monkeypatch) -> None:
    ...
    result = generate_sql("Show QA rows")
